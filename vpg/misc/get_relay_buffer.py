...
from algos.policy_base import OffPolicyAgent
from misc.utils import is_discrete
...
def get_space_size(space):
    if isinstance(space, Box):
        ...
        raise NotImplementedError("Please use Box or Discrete, not {}".format(type(space)))
...
def get_replay_buffer(policy, env, use_prioritized_rb=False, use_nstep_rb=False, n_step=1, size=None):
    ...
    if len(obs_shape) == 3:  # 判断obs_shape是否为三维
