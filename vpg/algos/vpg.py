...
from algos.policy_base import OnPolicyAgent
from policies.categorical_actor import CategoricalActor
...
class CriticV(tf.keras.Model):
    def __init__(self, state_shape, units, name='critic_v', hidden_activation='relu'):
        super(CriticV, self).__init__(name=name)
...
class VPG(OnPolicyAgent):
    def __init__(
            ...
            name="VPG",
            **kwargs):
        super(VPG, self).__init__(name=name, **kwargs)
        self._is_discrete = is_discrete

        if actor_critic is not None:
            self.actor_critic = actor_critic
            self.actor_critic_optimizer = tf.keras.optimizers.Adam(learning_rate=lr_actor)
            self.actor = None
            self.critic = None
        else:
            ...
            if actor is None:
                if is_discrete:
                    ...
                    self.actor = GaussianActor(state_shape, action_dim, max_action, actor_units,
            ...
            if critic is None:
                self.critic = CriticV(state_shape, critic_units, hidden_activation_critic)
            else:
                self.critic = critic
    ...
    def get_action_and_val(self, state, test=False):
        ...
        if is_single_input:
            state = np.expand_dims(state, axis=0).astype(np.float32)
        action, logp, v = self._get_action_and_v_body(state, test)
    ...
    @tf.function
    def _get_action_and_v_body(self, state, test=False):
        if self.actor_critic is not None:
          return self.actor_critic(state, test)
        else:
    ...
    def _train_critic_body(self, states, returns):
        with tf.device(self.device):   # /gpu:0
            with tf.GradientTape() as tape:
